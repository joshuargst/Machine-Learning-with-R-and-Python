...
sdev <- ADHD_Recipe$steps[[10]]$res$sdev
ADHD_Recipe$steps[[1]]	
...
ADHD_Cleaned %>% 
  ggplot(aes(PC1, PC2)) +
  geom_point(aes(color = Suicide), alpha = 0.7, size = 2)+
  labs(title="PCA from tidymodels")

ADHD_Cleaned %>% 
  ggplot(aes(PC1, PC2)) +
  geom_boxplot(aes(color = Suicide), alpha = 0.7, size = 2)+
  labs(title="PCA from tidymodels")

DataExplorer::plot_boxplot(ADHD_Cleaned, by = "Suicide")
```

```{r, fig.width=10, fig.height=8}
caret::featurePlot(ADHD_Cleaned[,c(2:6)], y = ADHD_Cleaned$Suicide)
DataExplorer::plot_correlation(ADHD_Cleaned)
GGally::ggpairs(ADHD_Cleaned, progress = F, aes(color = Suicide))+ theme_bw()
```
